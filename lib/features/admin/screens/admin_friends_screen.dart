...
class _AdminFriendsScreenState extends State<AdminFriendsScreen> {
  ...
  List<Map<String, dynamic>> myConnections = [];
  List<Map<String, dynamic>> pendingRequests = [];
  List<Map<String, dynamic>> exploreUsers = [];

  String activeTab = "connections"; // "connections", "requests", "explore"
  String searchQuery = "";
  ...
  Future<void> _fetchNetworkData() async {
    ...
    try {
      ...
      List<Map<String, dynamic>> connectionsList = [];
      ...
      for (var rel in (relations as List)) {
        ...
        if (status == 'accepted') {
          ...
          if (profile != null) {
            connectionsList.add({...profile, 'rel_id': relId});
          }
  ...
  Future<void> _acceptRequest(String relId) async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Connection accepted! ✅"), backgroundColor: Colors.green));
      }
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(isDecline ? "Request declined." : "Removed from network."),
  ...
  Future<void> _sendRequest(String receiverId) async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Connection request sent! 🚀"), backgroundColor: Colors.green));
      }
      await _fetchNetworkData();
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    if (activeTab == "connections") activeList = myConnections;
                ...
                child: Column(
                  ...
                  children: [
                        ...
                        children: [
                          ...
                          const Expanded(
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text("Academy Network", style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900, color: textDark, letterSpacing: -0.5)),
                                SizedBox(height: 4),
                                Text("Manage global connections and academy relationships.", style: TextStyle(fontSize: 12, color: textGrey, fontWeight: FontWeight.w500, height: 1.4)),
                              ],
                            ),
                          ),
                        ],
                    ...
                    Container(
                      ...
                      child: Row(
                        children: [
                          Expanded(child: _buildTabButton("Connected (${myConnections.length})", "connections")),
                    ...
                    filteredList.isNotEmpty
                        ? ListView.separated(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              final role = userItem['role'] ?? 'Member';
                                  ...
                                  children: [
                                    GestureDetector(
                                      ...
                                      child: CircleAvatar(
                                        ...
                                        child: avatar.isEmpty ? Text(name.isNotEmpty ? name[0] : 'U', style: const TextStyle(color: primaryPink, fontWeight: FontWeight.bold, fontSize: 20)) : null,
                                      ),
                                    ),
                                    const SizedBox(width: 16),
                                    Expanded(
                                      child: GestureDetector(
                                        ...
                                        child: Column(
                                          ...
                                          children: [
                                            ...
                                            Text("${role.toString().toUpperCase()} • Score: $score ⚡", style: const TextStyle(color: textGrey, fontSize: 11, fontWeight: FontWeight.w600)),
                                          ],
                                        ),
                                      ),
                                    ),
                                    ...
                                    // دکمه‌های عملیاتی بر اساس تب فعال
                                    if (activeTab == "connections")
                                      IconButton(
                                        ...
                                        onPressed: () => _removeOrDecline(userItem['rel_id']),
                                        tooltip: "Disconnect",
                                      )
                                        ...
                                        children: [
                                          ...
                                          SizedBox(
                                            ...
                                            child: IconButton(
                                              ...
                                              onPressed: () => _removeOrDecline(userItem['rel_id'], isDecline: true),
                                            ),
                                          ),
                                        ],
                                      )
                            ...
                            child: Center(
                              child: Column(
                                children: [
                                  ...
                                  const Text("No users found here.", style: TextStyle(color: textGrey, fontSize: 14, fontWeight: FontWeight.bold)),
                                ],
                              ),
                            ),
                          ),
