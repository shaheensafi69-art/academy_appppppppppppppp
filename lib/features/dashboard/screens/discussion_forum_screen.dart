  ...
  Future<void> _fetchForumPosts() async {
    ...
    try {
      final res = await supabase
          .from("discussion_posts") // جدول فرضی یا سفارشی انجمن گفتگو در دیتابیس
          .select("*")
          .order("created_at", ascending: false);

      if (res is List) {
        posts = res.map((p) => ForumPostItem.fromJson(p)).toList();
      }
    } catch (e) {
      // اگر جدول هنوز ایجاد نشده باشد، لیست خالی نمایش داده می‌شود تا ارور ندهد
      debugPrint("Forum posts table notice: $e");
    } finally {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("Discussion topic posted successfully!")),
        );
      }
    } catch (e) {
                                  ...
                                  Row(
                                    ...
                                    children: [
                                      Container(
                                        ...
                                        child: Text(post.studentName, style: const TextStyle(color: primaryPink, fontSize: 9, fontWeight: FontWeight.w900)),
                                      ),
                                      Text(post.createdAt.split('T')[0], style: const TextStyle(color: textGrey, fontSize: 9, fontWeight: FontWeight.bold)),
                                    ],
                                  ),
