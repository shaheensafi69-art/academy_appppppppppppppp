...
class _StudentAssignmentsScreenState extends State<StudentAssignmentsScreen> {
  ...
  Future<void> _fetchAllData() async {
    ...
    try {
      ...
      final signedClassIds =
          (myLogs as List?)
              ?.map((log) => log['class_group_id'] as String)
              .toList() ??
          [];
      ...
      final enrollments = await supabase
          .from("class_students")
          .select(
            "class_group_id, class_groups(id, class_name, meeting_link, signal_group_link)",
          )
          .eq("student_id", userId);
      ...
      if ((assignmentEnrollments as List).isNotEmpty) {
        final courseIds = assignmentEnrollments
            .map((e) => e['course_id'])
            .toList();
        ...
        assignments = (allAssignments as List).map((task) {
          final enrollment = assignmentEnrollments.firstWhere(
            (e) => e['course_id'] == task['course_id'],
            orElse: () => <String, dynamic>{},
          );
          final coursesObj = enrollment['courses'];
          final courseData = coursesObj is List
              ? (coursesObj.isNotEmpty ? coursesObj[0] : null)
              : coursesObj;
          final courseName = courseData?['title'] ?? 'Unknown Course';

          final submission = (submissions as List?)?.firstWhere(
                    ...
                    return GestureDetector(
                      onTap: () {
                        ...
                        Navigator.push(
                          ...
                          MaterialPageRoute(
                            builder: (_) =>
                                StudentAssignmentDetailScreen(assignment: task),
                          ),
