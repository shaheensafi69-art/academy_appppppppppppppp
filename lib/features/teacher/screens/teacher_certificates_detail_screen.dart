...
class TeacherCertificatesDetailScreen extends StatefulWidget {
  ...
  @override
  State<TeacherCertificatesDetailScreen> createState() =>
      _TeacherCertificatesDetailScreenState();
}

class _TeacherCertificatesDetailScreenState
    extends State<TeacherCertificatesDetailScreen> {
  ...
  Future<void> _handleSubmit() async {
    ...
    try {
      ...
      if (selectedFile != null) {
        ...
        final fileName =
            'cert_${selectedStudentId}_${DateTime.now().millisecondsSinceEpoch}.$fileExt';
        final filePath = fileName;

        fileUrl = await CloudflareStorageService.instance.upload(
          bucket: "certificates",
          path: filePath,
          bytes: fileBytes,
        );
  ...
  Widget build(BuildContext context) {
    ...
    final currentClass = classes.firstWhere(
      ...
      orElse: () => classes.isNotEmpty ? classes[0] : {},
    );
    final currentCourseTitle = (currentClass['courses'] is Map)
        ? currentClass['courses']['title']
        : 'Select Class & Course';
    ...
    final currentStudentName = students.isNotEmpty
        ? "${currentStudent['first_name']} ${currentStudent['last_name']}"
        : 'Select Student';
    final currentStudentEmail = students.isNotEmpty
        ? currentStudent['email']
        : '';
    final currentStudentAvatar = students.isNotEmpty
                      ...
                      PopupMenuButton<String>(
                        ...
                        itemBuilder: (context) {
                          return classes.map((c) {
                            final courseTitle = (c['courses'] is Map)
                                ? c['courses']['title']
                                : 'Course';
                        ...
                        onSelected: (val) {
                          setState(() {
                            ...
                            if (matchedClass['course_id'] != null) {
                              selectedCourseId = matchedClass['course_id']
                                  .toString();
                            }
                          });
                        },
                          ...
                          child: Row(
                            children: [
                              CircleAvatar(
                                radius: 16,
                                backgroundColor: lightPinkBg,
                                backgroundImage: currentStudentAvatar != null
                                    ? NetworkImage(currentStudentAvatar)
                                    : null,
                                child: currentStudentAvatar == null
                                    ? Text(
                                        currentStudentName.isNotEmpty
